...
const OnboardDealers = () => {
    ...
    const getRequests = async () => {
        ...
        try {
            const response = await axios.post(URLS.BASE_URI + URLS.ONBOARD_DETAILS,
                {
                },
                config
            );
            let responseData = response['data'];
            console.log("Onboard requests:", responseData['alldealers'])
            if (response.status === 200) {
                setCount(responseData['alldealers'].length)
                reArrageRequests(responseData['alldealers'])
            }
            updateLoading(false)
        } catch (error) {
        ...
        <SafeAreaView style={[GlobalStyles.container, { paddingTop: Platform.OS == 'android' ? 0 : 0 }]}>
            ...
            <View style={{ backgroundColor: COLORS.header, marginBottom: 20 }}>
                <View style={[GlobalStyles.header, {
                    ...
                    <Text style={[GlobalStyles.headerTitle, { marginRight: 25 }]}>Onboarded Dealers </Text>
                    <Text></Text>
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => {
                                    return <Request request={item} action={() => {
                                        navigation.navigate('onboardetail', { 'details': item })
                                    }} type='onboard' />
                                }} />
                        </View>
                    )
